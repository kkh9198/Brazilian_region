...
weeksum = orders_dataset.groupby('order_approved_at_weekday')['order_purchase_timestamp'].count()
weekavg = orders_dataset.groupby('order_approved_at_weekday')['actual_delivery_time'].mean()

weeks = ['Monday','Tuesday','Wednesday','Thursday',"Friday","Saturday","Sunday"]

weekavg = weekavg.agg(weeks)
...
plt.plot(weekavg.index,weekavg)
...
# 요일별 주문량(order_purchase_timestamp 기준)
plt.figure(figsize=(10,5))
plt.plot(weeksum.index,weeksum)
plt.title('amount of orders by weekday')
plt.xlabel('weekday')
plt.ylabel('orders')
plt.yticks(np.arange(5,15))
plt.xticks(np.arange(7))
